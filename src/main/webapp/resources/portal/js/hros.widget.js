					...
					if(widget != null){
						if(widget['error'] == 'E100'){
							...
							var options = {
								...
								url :  Util.fullUrl(HROS.CONFIG.basicDynamicUrl,widget['url']),
								width : widget['width'],
		...
		move : function(obj){
			obj.on('mousedown', '.move', function(e){
				...
				$(document).on('mousemove', function(e){
					...
					//_t = _t < 0 ? 0 : _t;
					_t = _t < 0 ? 0 : _t >= $(window).height() - 30 - HROS.CONFIG.headerHeight ? $(window).height() - 30 - HROS.CONFIG.headerHeight : _t;
					obj.css({
						left : _l,
						top : _t
					});
