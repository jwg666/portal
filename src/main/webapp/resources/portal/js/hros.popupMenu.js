		...
		*/
		app : function(obj){
			...
			$('.app-menu a[menu="edit"]').off('click').on('click', function(){
				$.dialog.open(HROS.CONFIG.basicActionUrl+'/papp.do?valIsapp=1&id=' + obj.attr('appid'), {
					id : 'editdialog',
		...
		papp : function(obj){
			...
			$('.papp-menu a[menu="edit"]').off('click').on('click', function(){
				$.dialog.open(HROS.CONFIG.basicActionUrl+'/papp.do?id=' + obj.attr('appid'), {
					id : 'editdialog',
			...
			$('.folder-menu a[menu="rename"]').off('click').on('click', function(){
				$.dialog({
					...
					content : editFolderDialogTemp({
						...
						'src_val' : obj.find('img').attr('src_val')
					}),
					ok : function(){
						if($('#folderName').val() != ''){
							$.ajax({
								type : 'POST',
								url : ajaxUrl,
								data : 'ac=updateFolder&name=' + $('#folderName').val() + '&icon=' + $('.folderSelector img').attr('src_val') + '&id=' + obj.attr('appid'),
								success : function(){
									HROS.app.get();
								}
							});
				...
				$('.fcDropdown_item').off('click').on('click', function(){
					$('.folderSelector img').attr('src', $(this).children('img').attr('src'))
											.attr('src_val', $(this).children('img').attr('src_val'))
											.attr('idx', $(this).children('img').attr('idx'));
					$('.fcDropdown').hide();
				});
				$('.popup-menu').hide();
			});
		...
		*/
		desk : function(){
			...
			if(!TEMP.popupMenuDesk){
				TEMP.popupMenuDesk = $('<div class="popup-menu desk-menu" style="z-index:9990;display:none"><ul><li><a menu="hideall" href="javascript:;">显示桌面</a></li><li style="border-bottom:1px solid #F0F0F0"><a menu="closeall" href="javascript:;">关闭所有应用</a></li><li class="hideInHome"><a href="javascript:;" >新建<b class="arrow">»</b></a><div class="popup-menu" style="display:none"><ul><li><b class="folder"></b><a menu="addfolder" href="javascript:;">新建文件夹</a></li><li><b class="customapp"></b><a menu="addpapp" href="javascript:;">新建私人应用</a></li></ul></div></li><!--li style="border-bottom:1px solid #F0F0F0"><b class="upload"></b><a menu="uploadfile" href="javascript:;">上传文件</a></li--><li><b class="themes"></b><a menu="themes" href="javascript:;">主题设置</a></li><li><b class="setting"></b><a menu="setting" href="javascript:;">桌面设置</a></li><li style="border-bottom:1px solid #F0F0F0" class="hideInHome"><a href="javascript:;">图标设置<b class="arrow">»</b></a><div class="popup-menu" style="display:none"><ul><li><b class="hook"></b><a menu="orderby" orderby="x" href="javascript:;">横向排列</a></li><li><b class="hook"></b><a menu="orderby" orderby="y" href="javascript:;">纵向排列</a></li></ul></div></li><li><a menu="logout" href="javascript:;">注销</a></li></ul></div>');
						...
						ok : function(){
							if($('#folderName').val() != ''){
								$.ajax({
									type : 'POST',
									url : ajaxUrl,
									data : 'ac=addFolder&name=' + $('#folderName').val() + '&icon=' + $('.folderSelector img').attr('src_val') + '&desk=' + HROS.CONFIG.desk,
									success : function(){
										HROS.app.get();
									}
								});
				...
				$('.desk-menu a[menu="addpapp"]').on('click', function(){
					...
					$.dialog.open(HROS.CONFIG.basicActionUrl+'/papp.do?desk=' + HROS.CONFIG.desk, {
						id : 'editdialog',
						title : '新建私人应用',
						width : 600,
						height : 450
				...
				$('.desk-menu a[menu="themes"]').on('click', function(){
					HROS.window.createTemp({
						appid : 'hoorayos-ztsz',
						title : '主题设置',
						url : HROS.CONFIG.basicActionUrl+'/wallpaper.do',
						width : 580,
						height : 520,
						isflash : false
					});
				...
				$('.desk-menu a[menu="setting"]').on('click', function(){
					HROS.window.createTemp({
						appid : 'hoorayos-zmsz',
						title : '桌面设置',
						url : HROS.CONFIG.basicActionUrl+'/desksetting.do',
						width : 750,
						height : 480,
						isflash : false
					});
			...
			if(HROS.CONFIG.desk == HROS.CONFIG.deskHome){
				TEMP.popupMenuDesk.find(".hideInHome").hide();
			}else{
				TEMP.popupMenuDesk.find(".hideInHome").show();
			}
